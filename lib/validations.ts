...
export const registerStep1Schema = z.object({
  fullName: z.string().min(3, 'Nama minimal 3 karakter').max(100, 'Nama maksimal 100 karakter').regex(/^[a-zA-Z\s'.-]+$/, 'Nama hanya boleh huruf & spasi'),
  phone: z.string().regex(/^(\+62|62|0)[0-9]{9,13}$/, 'Nomor HP tidak valid (contoh: 08123456789)'),
  password: z.string().min(8, 'Password minimal 8 karakter').regex(/[A-Z]/, 'Password harus mengandung huruf besar').regex(/[a-z]/, 'Password harus mengandung huruf kecil').regex(/[0-9]/, 'Password harus mengandung angka'),
...
export const loginSchema = z.object({
  phone: z.string().regex(/^(\+62|62|0)[0-9]{9,13}$/, 'Nomor HP tidak valid'),
...
export const productFormSchema = z.object({
  name: z.string().min(3, 'Nama produk minimal 3 karakter').max(120, 'Nama produk maksimal 120 karakter'),
  category: z.enum(['sayuran', 'buah', 'beras_padi', 'rempah', 'lainnya'], { message: 'Pilih kategori produk' }),
...
export const equipmentFormSchema = z.object({
  name: z.string().min(3, 'Nama alat minimal 3 karakter').max(120, 'Nama alat maksimal 120 karakter'),
  category: z.enum(['traktor', 'mesin_panen', 'pompa_air', 'drone', 'pupuk', 'bibit', 'pestisida', 'lainnya'], { message: 'Pilih kategori alat/bahan' }),
  description: z.string().max(2000, 'Deskripsi maksimal 2000 karakter').optional().or(z.literal('')),
...
// ─── BOOKING RETURN (Tahap 6 — Konfirmasi Pengembalian) ──────
