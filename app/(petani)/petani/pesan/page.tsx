// app/(petani)/petani/pesan/page.tsx
// Inbox untuk role petani — reuse komponen InboxClient dari pembeli
import { redirect } from 'next/navigation'
import { createServerSupabaseClient } from '@/lib/supabase/server'
import { InboxClient, type ConversationView } from '@/app/(pembeli)/pembeli/pesan/_components/InboxClient'

export const dynamic = 'force-dynamic'
export const metadata = { title: 'Pesan Saya' }

export default async function PetaniPesanPage() {
  const supabase = await createServerSupabaseClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login?redirect=/petani/pesan')
